...
void render_full(Viewport& viewport, ViewTransformer vtran, TScene scene) {
    scene.occlusion_cull(vtran);
    ...
    for (int ay = minmax_y[0]; ay < minmax_y[1]; ++ay) {
        for (int ax = 0; ax < viewport.width(); ++ax) {
            ...
            auto origin = vtran.to_world(absolute);
            auto dir = spt::vec3d({0.0, 0.0, -1.0});
            auto ointer = ray_mesh_intersection(origin, dir, mesh);
            if (ointer.has_value()) {
...
template <typename TScene>
void render_simple(Viewport& viewport, ViewTransformer vtran, TScene scene) {
    scene.occlusion_cull(vtran);
    auto& mesh = scene.mesh;
...
template <typename TScene>
void render_gouraud(Viewport& viewport, ViewTransformer vtran, TScene scene) {
    scene.occlusion_cull(vtran);
    auto& mesh = scene.mesh;
    ...
    for (int ay = minmax_y[0]; ay < minmax_y[1]; ++ay) {
        for (auto& face : mesh.surface) {
            double world_y = vtran.to_world(spt::vec2d({0.0, static_cast<double>(ay)}))[1];
...
template <typename TScene>
void render_phong(Viewport& viewport, ViewTransformer vtran, TScene scene) {
    scene.occlusion_cull(vtran);
    auto& mesh = scene.mesh;
    auto minmax_y = min_max_image_y(viewport, vtran, mesh);
    for (int ay = minmax_y[0]; ay < minmax_y[1]; ++ay) {
